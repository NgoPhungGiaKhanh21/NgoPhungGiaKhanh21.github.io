...
function findUserById(){
    const n = parseInt(prompt("Nhập n cho id user"));
    const user = users.find(user => user.id === n);

    if(user){
        console.log(user);
    }else{
        console.log("Không tìm thấy user với id:", n);
    }
}

//Nhập vào 1 chuỗi keyword. Hãy lấy ra email của các user có tên (first_name + last_name) chứa keyword.
function findEmail(){
    const keyword = prompt("Nhập từ khóa tìm kiếm người dùng");
    const emails = users.filter(user => (user.first_name + user.last_name).includes(keyword)).map(user => user.email);
    if(emails.length){
        console.log("Email của các user có tên chứa từ khóa '" + keyword + "':", emails);
    }else{
        console.log("Không có user nào có tên chứa từ khóa '" + keyword + "'.");
    }
}

//Đếm số lượng user có age > 50.
function countAge(){
    const count = users.filter(user => user.age > 50).length;
    console.log("Số lượng user có tuổi > 50 là:", count);
}

//Đếm số lượng user đã kết hôn và chưa kết hôn.
function marriedStatus(){
    const countMarried = users.filter(user => user.is_married).length;
    const notMarried = users.filter(user => !user.is_married).length;

    console.log(`Số lượng user đã kết hôn: ${countMarried}`);
    console.log(`Số lượng user chưa kết hôn: ${notMarried}`);
}

//Đếm số lượng user theo từng ngành nghề tương ứng.
function countJob(){
    const countJobs = users.reduce((a, b) => {
        a[b.job] = (a[b.job] || 0) + 1;
        return a;
    }, {}); 

    console.log("Số lượng user theo ngành nghề:", countJobs);
}
console.log("1. Nhập vào 1 số n. Hãy tìm 1 user có id bằng n");
findUserById();
console.log("2. Nhập vào 1 chuỗi keyword. Hãy lấy ra email của các user có tên (first_name + last_name) chứa keyword.");
findEmail();
console.log("3. Đếm số lượng user có age > 50.");
countAge();
console.log("4. Đếm số lượng user đã kết hôn và chưa kết hôn.");
marriedStatus();
console.log("5. Đếm số lượng user theo từng ngành nghề tương ứng.");
countJob();